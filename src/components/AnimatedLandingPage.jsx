  ...
  return (
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-green-700 via-green-600 to-green-800 p-6">
      ...
      <motion.div
        ...
        className="max-w-md w-full bg-green-700 backdrop-blur-md border border-white/10 rounded-2xl p-8 shadow-2xl"
        role="region"
        aria-label="Welcome card"
      >
        

        <header className="text-center rounded-md bg-green-700 mb-6">
          <motion.h1
        ...
        <main className="space-y-6">
          <motion.div
            ...
            className="px-4 py-3 bg-white/4 rounded-lg"
          >
          ...
          {/* CTA button with subtle pulse on hover */}
          <motion.div
            initial={{ scale: 1 }}
          ...
          >
            <motion.button
              onClick={handleCTA}
              aria-label={ctaText}
              className="w-full max-w-sm rounded-md py-3 px-6 font-semibold text-green-800 bg-white shadow hover:shadow-lg focus:outline-none focus:ring-4 focus:ring-white/30"
              whileHover={{ boxShadow: '0px 10px 30px rgba(255,255,255,0.06)' }}
              // add a looping subtle pulse on the button's background
              animate={{ boxShadow: ['0px 6px 18px rgba(0,0,0,0.06)', '0px 12px 30px rgba(0,0,0,0.08)', '0px 6px 18px rgba(0,0,0,0.06)'] }}
...
//   return <AnimatedLanding onCTA={handleCTA} ctaText="Create Account" />
// }
